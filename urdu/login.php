...
if (isset($_POST['login'])) {
    ...
    if ($usercheck_result->num_rows < 1) {
        $msg = '<div class="alert alert-danger" role="alert">
        <strong> Oops! Couldnt find your account, please proceed to signup </strong>
    </div>';
    } else {
        $login = "SELECT * FROM `users` WHERE `email`= '$email' AND `password` = '$password'";
        $login_result = $conn->query($login);
        if ($login_result == true) {
            $userdetails = $login_result->fetch_assoc();
            if ($userdetails['email'] == $_POST['email'] and $userdetails['password'] == $_POST['password']) {
                session_start();
                $_SESSION['user_id'] = $userdetails['user_id'];
                $_SESSION['user_name'] = $userdetails['fullname'];
